...
### [1] CONFIG · 2026-08-25 18:36:44 UTC

```
model: claude-sonnet-5 | effort: low | permission: bypassPermissions | cwd: /ai-inventor/aii_data/runs/run_GtJcfaBZUMxZ/3_invention_loop/iter_1/review_paper/review_paper
```

### [2] SYSTEM-USER prompt · 2026-08-25 18:36:52 UTC
...
### [3] HUMAN-USER prompt · 2026-08-25 18:36:52 UTC

```
Find a better admission control policy for overloaded request queues.
```

### [4] TOOL CALL — Write · 2026-08-25 18:38:05 UTC

```
File: /ai-inventor/aii_data/runs/run_GtJcfaBZUMxZ/3_invention_loop/iter_1/review_paper/review_paper/.terminal_claude_agent_struct_out.json

{
  "overall_assessment": "The paper proposes a genuinely clean idea -- repointing Adaptive Conformal Inference's threshold-tracking update onto the admission decision itself, so a distribution-free violation-rate guarantee attaches to admission control rather than to prediction-set coverage. The writing is disciplined, the experimental design (five regimes chosen specifically to stress the distribution-free claim, pre-registered tolerances, Holm-corrected paired bootstrap tests) is more rigorous than most systems papers, and the limitations section is unusually candid. However, the paper has one load-bearing flaw that a careful reviewer will not let pass: the entire empirical section runs on a self-generated re-implementation of the traffic simulator and all five policies, produced only because the upstream dataset and experiment-logging artifacts were empty at evaluation time. The paper reports this honestly, but the consequence is that no result in the paper was computed against the real Azure-trace-derived dataset that Sections 4.1 and the dataset artifact spend a full page describing -- the headline numbers characterize a stand-in the evaluator wrote from scratch, including its own version of the conformal policy under test. This turns the central empirical contribution into a self-consistency check rather than an independent evaluation, and it also raises an unaddressed conflict of interest: the same code path that implements the policy also implements the ground truth it is scored against. Combined with a Method section that states the ACI convergence claim by citation rather than proving (or even precisely stating) the theorem's assumptions in the paper's own notation, and a related-work section that stops citing conformal-risk-control literature at 2022, this is a promising paper that is not yet ready for acceptance in its current evidentiary form.",
  "strengths": [
    "The core idea -- applying ACI's online threshold-tracking update directly to an admission decision, rather than to a prediction set -- is a clean, well-motivated repurposing that the paper correctly identifies as absent from both the conformal-prediction and admission-control literatures.",
    "The five-regime experimental design (stationary, burst, drift, unannounced switch, adversarial) is specifically constructed to attack the claim rather than flatter it, which is the right adversarial posture for a distribution-free method.",
    "Statistical practice is above the systems-paper norm: pre-registered tolerance, Holm-Bonferroni correction across all 15 (regime, baseline) comparisons, moving-block bootstrap, and a pre-registered disconfirming threshold (50% value loss) for the matched-value comparison.",
    "The paper is honest about its central limitation (self-generated evaluation substituting for trace-derived data, only 3 seeds) rather than burying it, and the Discussion correctly interprets the two non-significant stationary-regime comparisons as expected rather than as a weakness to explain away.",
    "The value-aware admission layer is a sensible, low-risk extension (a knapsack re-ranking strictly inside the conformal eligibility set) that is evaluated separately from the safety claim, which is the right decomposition."
  ],
  "dimension_scores": [
    {
      "dimension": "soundness",
      "score": 2,
      "justification": "The paper's headline claim rests entirely on results computed by a self-generated stand-in evaluator whose data, ground-truth labels, and baseline implementations were all produced by the same pipeline that also implements and scores the proposed policy. The originally planned real-trace evaluation never ran. No formal theorem or proof sketch is given in the paper's own notation for the central guarantee; the reader is asked to accept it by citation to Gibbs & Candes. Three seeds is thin for the confidence intervals reported, and this is acknowledged but not remedied.",
      "improvements": [
        "Re-run the evaluation directly against the 210,000-row Azure-trace-derived dataset (art_fAlkDy9YEd-N) rather than the self-generated regime simulator, and report both the trace-derived numbers and the current self-generated numbers side by side so a reader can judge how much the substitution matters. EXPECTED IMPACT: this is the single highest-leverage fix available and would likely move soundness from 2 to 3-4, since it removes the central methodological objection.",
        "State the ACI convergence guarantee as a numbered assumption-and-conclusion statement in the paper's own notation (bounded loss, fixed step size eta, the exact long-run-average bound and its rate of convergence) rather than only in prose citing [2]. EXPECTED IMPACT: moderate; closes an easy 'the paper asserts a theorem it never states' critique.",
        "Increase seed count to at least 5 (the number the evaluation plan itself specifies as the threshold for the preferred over-seed bootstrap) or explicitly justify why 3 is sufficient given the block-bootstrap's known higher variance for short series. EXPECTED IMPACT: moderate; tightens the two borderline stationary-regime comparisons that the paper itself flags as sensitive to seed count."
      ]
    },
    {
      "dimension": "presentation",
      "score": 3,
      "justification": "The prose is clear, well-organized, and unusually precise about what is claimed versus what is shown. The limitations section is a genuine strength. Weaknesses are in omitted technical detail (risk-score construction, eta selection/sensitivity) and in a related-work section that is thorough on the four core conformal papers but does not engage with the post-2022 online-conformal literature.",
      "improvements": [
        "Add a short subsection or appendix specifying exactly how s(x_t) is computed (queue depth, per-endpoint service-time estimate, arrival-rate history -- the paper names these but gives no formula, weighting, or update frequency), since the guarantee's practical usefulness depends on this signal being cheap to compute in a real system, not just 'monotonic'.",
        "Report the value of eta used and a sensitivity analysis (e.g., a small grid) rather than treating it as a fixed, unstated hyperparameter -- the Discussion's own point about delayed-outcome re-tuning of eta makes clear the paper considers eta important, but no value or ablation appears anywhere in the results.",
        "Extend related work past 2022 (e.g., more recent strongly-adaptive / multi-window online conformal methods, and online conformal work explicitly aimed at control/decision-making rather than prediction sets) so a reader can judge whether the plain single-step ACI update chosen here is still the strongest available tracking rule."
      ]
    },
    {
      "dimension": "contribution",
      "score": 2,
      "justification": "The idea of applying ACI to admission control is a reasonable and, per the paper's own literature search, apparently unexplored combination -- but the paper's evidence for it currently rests on a synthetic self-check rather than a system or trace-grounded evaluation, which is what would let another group actually build on the result. As it stands the contribution is a plausible mechanism plus a simulation of that mechanism against a simulation of the baselines, which is a much weaker basis for adoption than the paper's own framing ('tested... against four baselines' grounded in a 'widely used public' trace) implies.",
      "improvements": [
        "Ground the significance claim in the real trace data (see soundness fix above) -- a result that survives contact with actual Azure Functions traffic statistics is a materially stronger basis for the 'others would build on this' claim than a re-implemented generator matched only on summary statistics.",
        "Add at least one baseline or ablation that isolates whether the gain over the fixed threshold and RL controller comes specifically from ACI's feedback structure versus any simple recency-weighted online threshold update (e.g., a naive exponential moving average of recent violation rate used to set a threshold) -- this would sharpen the claim that ACI's particular update, rather than 'any online correction', is doing the work.",
        "Discuss deployability constraints more concretely (e.g., what happens when many endpoints need per-endpoint thresholds and their outcome delays differ, which the Discussion raises as future work) -- as currently scoped the contribution is a single-endpoint, immediate-feedback mechanism, which is a narrower claim than the introduction's framing around 'a serverless function platform... thousands of distinct functions' suggests."
      ]
    }
  ],
  "critiques": [
      ...
      },
    {
      "category": "evidence",
      "severity": "minor",
      "description": "Statistical power is limited: only 3 independent seeds per (policy, regime) cell, below the 5 the evaluation plan itself identifies as the threshold for its preferred over-seed bootstrap, forcing a fallback to a block-over-time bootstrap. The paper's own Limitations section notes this could plausibly flip one or both of the two non-significant stationary-regime comparisons.",
      "suggested_action": "Increase to at least 5 seeds per cell (as the evaluation plan originally specified) to use the intended over-seed bootstrap and tighten the confidence intervals on the two borderline stationary-regime comparisons."
    },
    {
      "category": "clarity",
      "severity": "minor",
      "description": "The risk score s(x_t) is described only qualitatively ('current queue depth, a coarse per-endpoint service-time estimate, and recent arrival-rate history') with no formula, update cadence, or computational cost analysis, despite the paper's practical claim that it is 'cheap' -- a reader cannot judge how cheap, or reproduce it, from the text given.",
      "suggested_action": "Give the exact functional form (or pseudocode) used to compute s(x_t) in the artifact, its computational complexity per request, and how it is updated online, either in the Method section or an appendix."
    },
    {
      "category": "clarity",
      "severity": "minor",
      "description": "The step size eta is never given a numeric value anywhere in the paper, nor is any sensitivity analysis over it provided, even though the Discussion explicitly identifies eta as the parameter that would need re-tuning under delayed outcome observation -- a reader cannot assess how sensitive the headline results are to this choice.",
      "suggested_action": "Report the eta value(s) used in Section 5 and add a small sensitivity sweep (e.g., 3-5 values) showing how MAD-from-target and transient spike size trade off against eta, at least for the regime-switch and adversarial regimes where responsiveness matters most."
    },
    {
      "category": "novelty",
      "severity": "minor",
      "description": "Related work stops citing the online/adaptive conformal literature at 2022 (Gibbs & Candes; Feldman et al.). Given the field's activity since then on strongly-adaptive and multi-window online conformal tracking, the paper cannot currently claim it has selected the strongest available threshold-tracking rule to repoint at admission control, only that it used a specific well-known one.",
      "suggested_action": "Add a paragraph acknowledging post-2022 online conformal tracking variants and either justify the plain ACI update as the appropriate baseline choice for this first application, or note the newer variants as a natural extension (this can be folded into the existing Future Work paragraph)."
    },
    {
      "category": "scope",
      "severity": "minor",
      "description": "The introduction motivates the problem at the scale of 'a function-as-a-service provider processes millions of invocations across thousands of distinct functions,' but the method and evaluation operate a single scalar threshold shared across all traffic, and multi-endpoint/multi-tenant thresholding under a joint budget is explicitly deferred to future work. The gap between the motivating scale and the evaluated scope is large and not flagged until the very end of the paper.",
      "suggested_action": "Either narrow the introduction's motivating scenario to match the single-threshold scope actually evaluated, or move the multi-endpoint caveat earlier (e.g., into the Introduction or Method) so the reader is not left assuming per-function guarantees that the paper does not test."
    }
  ],
  "score": 4,
  "confidence": 4
...
### [5] TOOL RESULT — Write · 2026-08-25 18:38:05 UTC

```
{"type": "create", "filePath": "/ai-inventor/aii_data/runs/run_GtJcfaBZUMxZ/3_invention_loop/iter_1/review_paper/review_paper/.terminal_claude_agent_struct_out.json", "content": "{\n  \"overall_assessment\": \"The paper proposes a genuinely clean idea -- repointing Adaptive Conformal Inference's threshold-tracking update onto the admission decision itself, so a distribution-free violation-rate guarantee attaches to admission control rather than to prediction-set coverage. The writing is disciplined, the experimental design (five regimes chosen specifically to stress the distribution-free claim, pre-registered tolerances, Holm-corrected paired bootstrap tests) is more rigorous than most systems papers, and the limitations section is unusually candid. However, the paper has one load-bearing flaw that a careful reviewer will not let pass: the entire empirical section runs on a self-generated re-implementation of the traffic simulator and all five policies, produced only because the upstream dataset and experiment-logging artifacts were empty at evaluation time. The paper reports this honestly, but the consequence is that no result in the paper was computed against the real Azure-trace-derived dataset that Sections 4.1 and the dataset artifact spend a full page describing -- the headline numbers characterize a stand-in the evaluator wrote from scratch, including its own version of the conformal policy under test. This turns the central empirical contribution into a self-consistency check rather than an independent evaluation, and it also raises an unaddressed conflict of interest: the same code path that implements the policy also implements the ground truth it is scored against. Combined with a Method section that states the ACI convergence claim by citation rather than proving (or even precisely stating) the theorem's assumptions in the paper's own notation, and a related-work section that stops citing conformal-risk-control literature at 2022, this is a promising paper that is not yet ready for acceptance in its current evidentiary form.\",\n  \"strengths\": [\n    \"The core idea -- applying ACI's online threshold-tracking update directly to an admission decision, rather than to a prediction set -- is a clean, well-motivated repurposing that the paper correctly identifies as absent from both the conformal-prediction and admission-control literatures.\",\n    \"The five-regime experimental design (stationary, burst, drift, unannounced switch, adversarial) is specifically constructed to attack the claim rather than flatter it, which is the right adversarial posture for a distribution-free method.\",\n    \"Statistical practice is above the systems-paper norm: pre-registered tolerance, Holm-Bonferroni correction across all 15 (regime, baseline) comparisons, moving-block bootstrap, and a pre-registered disconfirming threshold (50% value loss) for the matched-value comparison.\",\n    \"The paper is honest about its central limitation (self-generated evaluation substituting for trace-derived data, only 3 seeds) rather than burying it, and the Discussion correctly interprets the two non-significant stationary-regime comparisons as expected rather than as a weakness to explain away.\",\n    \"The value-aware admission layer is a sensible, low-risk extension (a knapsack re-ranking strictly inside the conformal eligibility set) that is evaluated separately from the safety claim, which is the right decomposition.\"\n  ],\n  \"dimension_scores\": [\n    {\n      \"dimension\": \"soundness\",\n      \"score\": 2,\n      \"justification\": \"The paper's headline claim rests entirely on results computed by a self-generated stand-in evaluator whose data, ground-truth labels, and baseline implementations were all produced by the same pipeline that also implements and scores the proposed policy. The originally planned real-trace evaluation never ran. No formal theorem or proof sketch is given in the paper's own notation for the central guarantee; the reader is asked to accept it by citation to Gibbs & Candes. Three seeds is thin for the confidence intervals reported, and this is acknowledged but not remedied.\",\n      \"improvements\": [\n        \"Re-run the evaluation directly against the 210,000-row Azure-trace-derived dataset (art_fAlkDy9YEd-N) rather than the self-generated regime simulator, and report both the trace-derived numbers and the current self-generated numbers side by side so a reader can judge how much the substitution matters. EXPECTED IMPACT: this is the single highest-leverage fix available and would likely move soundness from 2 to 3-4, since it removes the central methodological objection.\",\n        \"State the ACI convergence guarantee as a numbered assumption-and-conclusion statement in the paper's own notation (bounded loss, fixed step size eta, the exact long-run-average bound and its rate of convergence) rather than only in prose citing [2]. EXPECTED IMPACT: moderate; closes an easy 'the paper asserts a theorem it never states' critique.\",\n        \"Increase seed count to at least 5 (the number the evaluation plan itself specifies as the threshold for the preferred over-seed bootstrap) or explicitly justify why 3 is sufficient given the block-bootstrap's known higher variance for short series. EXPECTED IMPACT: moderate; tightens the two borderline stationary-regime comparisons that the paper itself flags as sensitive to seed count.\"\n      ]\n    },\n    {\n      \"dimension\": \"presentation\",\n      \"score\": 3,\n      \"justification\": \"The prose is clear, well-organized, and unusually precise about what is claimed versus what is shown. The limitations section is a genuine strength. Weaknesses are in omitted technical detail (risk-score construction, eta selection/sensitivity) and in a related-work section that is thorough on the four core conformal papers but does not engage with the post-2022 online-conformal literature.\",\n      \"improvements\": [\n        \"Add a short subsection or appendix specifying exactly how s(x_t) is computed (queue depth, per-endpoint service-time estimate, arrival-rate history -- the paper names these but gives no formula, weighting, or update frequency), since the guarantee's practical usefulness depends on this signal being cheap to compute in a real system, not just 'monotonic'.\",\n        \"Report the value of eta used and a sensitivity analysis (e.g., a small grid) rather than treating it as a fixed, unstated hyperparameter -- the Discussion's own point about delayed-outcome re-tuning of eta makes clear the paper considers eta important, but no value or ablation appears anywhere in the results.\",\n        \"Extend related work past 2022 (e.g., more recent strongly-adaptive / multi-window online conformal methods, and online conformal work explicitly aimed at control/decision-making rather than prediction sets) so a reader can judge whether the plain single-step ACI update chosen here is still the strongest available tracking rule.\"\n      ]\n    },\n    {\n      \"dimension\": \"contribution\",\n      \"score\": 2,\n      \"justification\": \"The idea of applying ACI to admission control is a reasonable and, per the paper's own literature search, apparently unexplored combination -- but the paper's evidence for it currently rests on a synthetic self-check rather than a system or trace-grounded evaluation, which is what would let another group actually build on the result. As it stands the contribution is a plausible mechanism plus a simulation of that mechanism against a simulation of the baselines, which is a much weaker basis for adoption than the paper's own framing ('tested... against four baselines' grounded in a 'widely used public' trace) implies.\",\n      \"improvements\": [\n        \"Ground the significance claim in the real trace data (see soundness fix above) -- a result that survives contact with actual Azure Functions traffic statistics is a materially stronger basis for the 'others would build on this' claim than a re-implemented generator matched only on summary statistics.\",\n        \"Add at least one baseline or ablation that isolates whether the gain over the fixed threshold and RL controller comes specifically from ACI's feedback structure versus any simple recency-weighted online threshold update (e.g., a naive exponential moving average of recent violation rate used to set a threshold) -- this would sharpen the claim that ACI's particular update, rather than 'any online correction', is doing the work.\",\n        \"Discuss deployability constraints more concretely (e.g., what happens when many endpoints need per-endpoint thresholds and their outcome delays differ, which the Discussion raises as future work) -- as currently scoped the contribution is a single-endpoint, immediate-feedback mechanism, which is a narrower claim than the introduction's framing around 'a serverless function platform... thousands of distinct functions' suggests.\"\n      ]\n    }\n  ],\n  \"critiques\": [\n    {\n      \"category\": \"evidence\",\n      \"severity\": \"major\",\n      \"description\": \"The reported results were not produced against the real, trace-derived dataset the paper spends a full subsection (4.1) and a full artifact (art_fAlkDy9YEd-N) describing. Because that dataset and the experiment-logging artifact were empty at evaluation time, eval.py self-generated its own multi-regime traffic simulator and re-implemented all five policies (including the conformal controller under test) from scratch. The paper discloses this in a limitations paragraph, but the framing throughout the Introduction, Experimental Setup, and Results ('four of the five regimes are grounded in a 210,000-request dataset...') reads, until that disclosure, as though the reported Table 1 numbers come from that dataset. They do not.\",\n      \"suggested_action\": \"Either (a) re-run the full evaluation against the actual trace-derived dataset and report those numbers as the headline result, moving the current self-generated numbers to an appendix as a robustness check, or (b) if that is not feasible in this iteration, restructure the paper so the self-generated nature of the evaluation is stated in the abstract and at the top of Section 5, not only in the Limitations paragraph near the end -- a reader should not need to reach the Discussion to learn that the headline table is not evaluated on the dataset the paper spent a subsection constructing.\"\n      },\n    {\n      \"category\": \"rigor\",\n      \"severity\": \"major\",\n      \"description\": \"The eval.py artifact that produces the paper's results implements both the policies under test (including the proposed conformal controller) and the traffic/label generator used to score them, in the same self-contained script, with no independently produced ground truth. This creates a structural risk that the evaluator's assumptions about what 'a violation' or 'a regime' looks like are baked identically into both the data-generating process and the policies being compared, which could inflate or bias the reported gaps in ways an externally sourced trace would not.\",\n      \"suggested_action\": \"Have an independent script (or, at minimum, a script written before and frozen against the policy implementations) generate the traffic and ground-truth labels, and only then run the five policies against that frozen dataset, so the comparison is not self-referential. Report a hash or fixed seed manifest so a third party could reproduce the exact traffic sequence independently of the evaluation code.\"\n      },\n    {\n      \"category\": \"methodology\",\n      \"severity\": \"major\",\n      \"description\": \"The paper states the ACI convergence property only by natural-language paraphrase and citation ('Gibbs and Candes [2] show that this single-parameter update achieves the target long-run average loss rate...'), with no theorem statement, no explicit assumptions (e.g., loss boundedness in [0,1], fixed vs. adaptive step size, the exact non-asymptotic bound), and no proof sketch connecting the admission-control setting's specific loss (a delayed binary SLO-violation indicator) to the assumptions under which the cited result actually holds.\",\n      \"suggested_action\": \"Add a short formal subsection in Section 3 stating the guarantee as an explicit theorem (with citation) in the paper's own notation for lambda_t, y_t, alpha, and eta, including the finite-sample bound on |average(y_t) - alpha| over a window of length T as a function of eta and the loss range, and explicitly verify that the admission-control setting (bounded-delay binary outcome) satisfies the theorem's preconditions.\"\n      },\n    {\n      \"category\": \"evidence\",\n      \"severity\": \"minor\",\n      \"description\": \"Statistical power is limited: only 3 independent seeds per (policy, regime) cell, below the 5 the evaluation plan itself identifies as the threshold for its preferred over-seed bootstrap, forcing a fallback to a block-over-time bootstrap. The paper's own Limitations section notes this could plausibly flip one or both of the two non-significant stationary-regime comparisons.\",\n      \"suggested_action\": \"Increase to at least 5 seeds per cell (as the evaluation plan originally specified) to use the intended over-seed bootstrap and tighten the confidence intervals on the two borderline stationary-regime comparisons.\"\n    },\n    {\n      \"category\": \"clarity\",\n      \"severity\": \"minor\",\n      \"description\": \"The risk score s(x_t) is described only qualitatively ('current queue depth, a coarse per-endpoint service-time estimate, and recent arrival-rate history') with no formula, update cadence, or computational cost analysis, despite the paper's practical claim that it is 'cheap' -- a reader cannot judge how cheap, or reproduce it, from the text given.\",\n      \"suggested_action\": \"Give the exact functional form (or pseudocode) used to compute s(x_t) in the artifact, its computational complexity per request, and how it is updated online, either in the Method section or an appendix.\"\n    },\n    {\n      \"category\": \"clarity\",\n      \"severity\": \"minor\",\n      \"description\": \"The step size eta is never given a numeric value anywhere in the paper, nor is any sensitivity analysis over it provided, even though the Discussion explicitly identifies eta as the parameter that would need re-tuning under delayed outcome observation -- a reader cannot assess how sensitive the headline results are to this choice.\",\n      \"suggested_action\": \"Report the eta value(s) used in Section 5 and add a small sensitivity sweep (e.g., 3-5 values) showing how MAD-from-target and transient spike size trade off against eta, at least for the regime-switch and adversarial regimes where responsiveness matters most.\"\n    },\n    {\n      \"category\": \"novelty\",\n      \"severity\": \"minor\",\n      \"description\": \"Related work stops citing the online/adaptive conformal literature at 2022 (Gibbs & Candes; Feldman et al.). Given the field's activity since then on strongly-adaptive and multi-window online conformal tracking, the paper cannot currently claim it has selected the strongest available threshold-tracking rule to repoint at admission control, only that it used a specific well-known one.\",\n      \"suggested_action\": \"Add a paragraph acknowledging post-2022 online conformal tracking variants and either justify the plain ACI update as the appropriate baseline choice for this first application, or note the newer variants as a natural extension (this can be folded into the existing Future Work paragraph).\"\n    },\n    {\n      \"category\": \"scope\",\n      \"severity\": \"minor\",\n      \"description\": \"The introduction motivates the problem at the scale of 'a function-as-a-service provider processes millions of invocations across thousands of distinct functions,' but the method and evaluation operate a single scalar threshold shared across all traffic, and multi-endpoint/multi-tenant thresholding under a joint budget is explicitly deferred to future work. The gap between the motivating scale and the evaluated scope is large and not flagged until the very end of the paper.\",\n      \"suggested_action\": \"Either narrow the introduction's motivating scenario to match the single-threshold scope actually evaluated, or move the multi-endpoint caveat earlier (e.g., into the Introduction or Method) so the reader is not left assuming per-function guarantees that the paper does not test.\"\n    }\n  ],\n  \"score\": 4,\n  \"confidence\": 4\n}\n", "structuredPatch": [], "originalFile": null, "userModified": false}
```

### [6] ASSISTANT · 2026-08-25 18:38:08 UTC

```
Review complete — the JSON output has been written with a score of 4/10 (Borderline Reject), driven primarily by the fact that the reported results come from a self-generated stand-in simulator/evaluator rather than the real Azure-trace dataset the paper describes, plus a missing formal statement of the ACI guarantee. The paper's core idea and experimental rigor otherwise show genuine promise.
```
