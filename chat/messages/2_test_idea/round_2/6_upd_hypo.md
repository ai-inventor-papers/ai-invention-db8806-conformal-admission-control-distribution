...
### [1] CONFIG · 2026-08-25 19:23:29 UTC

```
model: claude-sonnet-5 | effort: low | permission: bypassPermissions | cwd: /ai-inventor/aii_data/runs/run_GtJcfaBZUMxZ/3_invention_loop/iter_2/upd_hypo/upd_hypo
```

### [2] SYSTEM-USER prompt · 2026-08-25 19:23:37 UTC
...
### [3] HUMAN-USER prompt · 2026-08-25 19:23:37 UTC

```
Find a better admission control policy for overloaded request queues.
```

### [4] TOOL CALL — Write · 2026-08-25 19:25:07 UTC

```
File: /ai-inventor/aii_data/runs/run_GtJcfaBZUMxZ/3_invention_loop/iter_2/upd_hypo/upd_hypo/.terminal_claude_agent_struct_out.json

{
  ...
  "motivation": "Unchanged from iteration 1: queueing-theoretic/index-based policies (Nino-Mora 2002) give guarantees conditional on a fitted distributional model; RL/learned controllers (TopFull, SIGCOMM 2024) adapt empirically but offer no finite-sample safety guarantee at all. Online conformal risk control (Angelopoulos et al. 2024) and Adaptive Conformal Inference (Gibbs & Candes 2021) offer a genuinely different guarantee type -- distribution-free, no exchangeability assumption -- that a targeted search still finds unapplied to systems admission control. Iteration 2 sharpens this motivation with a caveat the theorem itself supplies: the guarantee's non-asymptotic bound is only non-vacuous when enough requests are actually admitted, so the paradigm's strongest selling point (a hard number regardless of the true process) has a real, reportable failure mode -- sparse admission under an extreme adversarial score distribution -- that the paper must characterize rather than paper over.",
  "assumptions": [
    "A real-valued risk score s(x) is available cheaply at admission time and is at least weakly informative; iteration-2's real-trace O(1) formula (function-level service-time gap, local-vs-baseline arrival rate, queue depth, logistic-blended) satisfies this on stationary/burst/drift/regime_switch, while the adversarial regime deliberately uses a bimodal, non-model-separable score by construction.",
    "The true outcome becomes observable within a bounded delay after the admission decision; iteration-2's dataset satisfies this (outcomes computed from realized service time, effectively instantaneous), and this remains an explicit, now-formalized precondition of the theorem -- a production deployment with a longer confirmation delay would need eta re-tuned against it, widening the effective window the bound applies to.",
    "The per-decision loss reduces to a single bounded scalar (binary SLO-violation indicator); retained unchanged, with the theorem now explicit that the update is admission-CONDITIONAL (rejected requests carry lambda forward unchanged, unlike the original ACI setting's every-step observation).",
    "Admitting/rejecting one request does not retroactively change already-admitted requests' outcomes in a way that breaks the per-decision feedback assumption; retained as an approximation to be understood per-decision-given-trajectory, not causal independence.",
    "Request value is estimable at admission time for the knapsack tie-break; iteration-2 evidence NARROWS this: on the real trace, a value proxy correlated with the same risk score that determines eligibility gives the knapsack layer no significant advantage over FCFS, so the assumption should now be stated as requiring a value signal genuinely INDEPENDENT of the eligibility-determining risk score (e.g., real per-tenant billing weight) for a value gain to be expected, not merely 'value is available'.",
    "NEW (from iteration-2 evidence): the theorem's finite-sample bound is only practically informative when the number of ADMITTED requests N_T is large enough that B/(eta*N_T) <= 1; a regime or score design that causes very sparse admission (as the current adversarial regime does) makes any headline comparison computed there evidentially fragile regardless of which policy 'wins' the raw MAD number."
  ],
  "investigation_approach": "Phase 0/1/2/3 retained from prior iterations (mechanism check across regimes; baselines including a properly-labeled frozen supervised stand-in; score-quality sensitivity; value-aware knapsack layer), now run against the real Azure-trace dataset as PRIMARY (closing the iteration-1 self-referential-evaluation gap) with the self-generated simulator demoted to a secondary, labeled robustness check.\n\nNEW for the next iteration, directly targeting reviewer MAJOR findings: (a) Redesign the adversarial regime (more rows, and/or less extreme bimodal score separation so a meaningful fraction of requests remain admitted) so the theorem's own non-vacuity precondition (B/(eta*N_T) <= 1) holds at the evaluated eta grid, before reporting any adversarial-regime comparison as a headline win; alternatively, if the regime is kept as-is, every mention of the adversarial-regime result outside the Limitations section must carry the small-N/vacuous-bound caveat inline. (b) Rename the 'frozen RL baseline' consistently to 'frozen supervised stand-in for a learned controller' throughout all artifacts and the paper, and consider adding a genuinely retrained/continually-updated learned baseline as a sixth policy to test whether the drift/adversarial gaps narrow under a learned controller that CAN adapt. (c) Add a structural explanation (in the next evaluation artifact's own output, not just prose) for why the fixed-threshold and misspecified-index baselines produce near-identical MAD -- confirm/quantify that both reduce to static rules absent a feedback signal. (d) Run one of the two cited post-2022 online conformal variants (strongly-adaptive multi-window [10], or parameter-free group-conditional [11]) as an added condition in the eta-sensitivity comparison, directly testing whether it removes the need to hand-tune eta per regime, as the Discussion currently speculates. (e) Set alpha relative to each regime's own natural violation rate (or otherwise restrict pass/fail tolerance claims to regimes where the target is a genuine constraint) so the aggregate pass-rate reporting is not dominated by the three structurally-unattainable regimes.",
  "success_criteria": "Retained from iteration 2, with the vacuous-bound caveat now load-bearing: CONFIRMING evidence is (1) conformal control passing the 3pp tolerance and beating non-oracle baselines Holm-significantly in regimes where the target is a genuine constraint (natural rate >= alpha) AND where the admitted count N_T keeps the theorem's bound non-vacuous (B/(eta*N_T) <= 1) -- a win obtained only via a sparse-admission regime does not count as confirming evidence for the headline claim without the caveat attached; (2) at matched violation rate in the stationary regime, conformal's accepted value is not worse than, and can exceed, baselines' (iteration 2: statistically indistinguishable from fixed-threshold/supervised-stand-in, ~13% better than misspecified-index, CI excludes 0 -- CONFIRMED); (3) the value-aware knapsack layer shows a significant value gain over FCFS only when tested with a value signal independent of the eligibility risk score (iteration 2's correlated proxy showed a null result -- this criterion is NOT yet met and remains open). DISCONFIRMING evidence, now made concrete by iteration 2: a claimed regime-level win that only appears where N_T is small enough to make the theorem's own bound vacuous does not, on its own, disconfirm the mechanism, but it also cannot be reported as confirming it without the redesign or caveat in (a) above. PARTIAL confirmation (matches iteration-2 status): the guarantee holds robustly and provably wins over frozen baselines specifically in regimes where the target is a genuine constraint and admission is not pathologically sparse (drift); it is untestable by a fixed global tolerance in regimes whose natural rate already sits far from target (stationary/burst/regime_switch), which is a precisely-characterized methodological finding about the evaluation criterion, not a policy failure; and it currently rests on an under-scrutinized win (adversarial) that must be either strengthened by redesign or explicitly downgraded to a caveated, non-headline result in both text and future evaluation design.",
  "related_works": [
    "Angelopoulos, A. N., Bates, S., et al. (2024) 'Conformal Risk Control' -- ICLR 2024 / arXiv:2208.02814. Core mechanism repurposed for admission-decision thresholding; the paper itself does not address queueing or systems SLOs.",
    "Gibbs, I. & Candes, E. (2021) 'Adaptive Conformal Inference Under Distribution Shift' -- NeurIPS 2021 / arXiv:2106.00170. Source of the lambda_t online gradient update; iteration 2's paper states an explicit theorem adapting this to the admission-CONDITIONAL setting (threshold frozen on rejection) and to a bound over admitted count N_T rather than raw arrivals -- a genuine, reported departure from the original result's setting.",
    "Nino-Mora, J. (2002) 'Dynamic allocation indices for restless projects and queueing admission control' -- Mathematical Programming 93. Model-based near-optimality guarantee under an assumed birth-death process; iteration 2's misspecified-index baseline instantiates this and (new finding) converges to near-identical MAD as a naive fixed threshold on the real trace absent a feedback signal, worth explaining structurally in the next iteration.",
    "Park, J. et al. (2024) 'TopFull' -- ACM SIGCOMM 2024. Real deep-RL admission/rate controller with no formal guarantee; iteration 2's 'frozen RL baseline' is NOT this system but a frozen logistic-regression stand-in, and reviewer feedback requires this distinction be made explicit and consistent throughout the paper rather than only in Limitations.",
    "Shahrad, M. et al. (2020) 'Serverless in the Wild' -- USENIX ATC 2020. Source trace for the 210,000-row real dataset (art_fAlkDy9YEd-N) now used as the primary evaluation, closing iteration-1's self-referential-evaluation gap.",
    "Bhatnagar, A. et al. (2023) 'Improved Online Conformal Prediction via Strongly Adaptive Online Learning' -- ICML 2023 / arXiv:2302.07869, and Bharti, B. et al. (2026) 'Parameter-Free and Group Conditional Online Conformal Prediction' -- arXiv:2606.00419. Cited as motivated future-work variants in iteration 2's paper; reviewer feedback recommends actually running one of these as an added condition, since the paper's generality claim currently covers only the single-eta plain-ACI update it evaluates."
  ],
  "inspiration": "Unchanged from iteration 1: CONCEPTUAL -- reframing admission thresholding from a model-based question into a pure feedback-control question over a scalar. PROCEDURAL -- importing the observe-outcome/compute-loss/gradient-step/update-threshold loop from ACI's prediction-interval workflow, repointed at admit/reject with an SLO-violation loss. METHODOLOGICAL -- combining Conformal Risk Control's threshold-selection guarantee with Gibbs & Candes' non-exchangeable online update, plus a queueing-specific score function and a value-aware knapsack layer with no direct analogue in the source literature; iteration 2 adds the explicit admission-CONDITIONAL adaptation of the update rule (threshold frozen, not updated, on rejection) as a documented, theorem-level departure from the source setting.",
  "terms": [
    {"term": "Admission control", "definition": "The policy decision of whether to accept or reject an incoming request into a queue/service system."},
    {"term": "Conformal risk control", "definition": "A method (Angelopoulos et al. 2024) for choosing a decision threshold that provably controls the expected value of a bounded, monotone loss at a target level, with a finite-sample, distribution-free guarantee."},
    {"term": "Adaptive Conformal Inference (ACI)", "definition": "An online procedure (Gibbs & Candes 2021) that updates a single threshold via a bounded gradient step after each observed outcome, provably achieving a target long-run violation frequency without exchangeability. Iteration 2's admission-control adaptation makes the update admission-conditional: the threshold is frozen (not updated) on any step where a request is rejected and no outcome is observed."},
    {"term": "SLO (Service Level Objective) violation", "definition": "The event that an admitted request's realized latency exceeds its target threshold."},
    {"term": "Distribution-free guarantee", "definition": "A statistical guarantee holding for any underlying data-generating process, in contrast to queueing-theoretic guarantees conditional on an assumed distributional model. Iteration 2 clarifies this guarantee is a bound over the number of ADMITTED requests N_T, not raw arrivals, and is vacuous (B/(eta*N_T) > 1) when too few requests are admitted."},
    {"term": "Risk score s(x)", "definition": "A cheap, possibly miscalibrated, real-valued function used only to rank requests by estimated SLO-violation risk; the conformal threshold, not the score's calibration, provides the formal guarantee."},
    {"term": "Value-aware knapsack tie-break", "definition": "Within the conformally-eligible set, admitting the highest-value requests first up to capacity rather than FCFS; iteration-2 real-trace evidence shows this yields no significant value gain when the value signal is correlated with the eligibility-determining risk score."},
    {"term": "Frozen supervised stand-in for a learned controller", "definition": "Iteration-2 renaming (per reviewer feedback) of what was previously called the 'frozen RL baseline': a single frozen logistic-regression contextual-bandit-style controller trained once on stationary traffic, distinct from and weaker than a genuine deep-RL system such as TopFull."}
  ],
  "summary": "This hypothesis proposes ADMISSION CONTROL VIA ONLINE CONFORMAL RISK CONTROL, evaluated in iteration 2 against a real 210,000-row Azure Functions trace as the primary dataset (closing iteration 1's self-referential-evaluation gap). The controller is the only non-oracle policy passing the pre-registered tolerance in the drift regime (a clean, statistically significant win) and shows a materially lower deviation from target in the adversarial regime against baselines including a mislabeled 'frozen RL' baseline now correctly identified as a frozen logistic-regression stand-in -- but that adversarial-regime win rests on very few admitted requests and falls inside the theorem's own vacuous-bound precondition, so it must be caveated or re-earned via regime redesign rather than reported as a clean headline result. A key methodological finding is that a single global tolerance line cannot fairly test policies in regimes whose natural violation rate already sits far from the target -- even a hindsight-optimal oracle fails there by construction -- reframing what 'distribution-free control' can be shown to mean empirically. The value-aware knapsack layer shows no significant gain over FCFS with a risk-score-correlated value proxy, a genuine negative result retained rather than smoothed over.",
  "_relation_rationale": "Same ACI frame; real-trace evidence sharpens claims, exposes vacuous-bound caveat on adversarial win, fixes RL-baseline mislabeling",
  "relation_rationale": "Same ACI frame; narrowed adversarial-win claim to its vacuous-bound caveat, fixed RL-baseline mislabeling",
  "confidence_delta": "unchanged",
  "key_changes": [
    "Attached the small-N/vacuous-bound caveat (B/(eta*N_T) > 1) directly to the adversarial-regime win as a first-class evidentiary limitation, not just a Section-5.3 footnote -- the hypothesis now states this win may not be reported as a clean headline result without either the caveat or a regime redesign that keeps the bound non-vacuous.",
    "Renamed the 'frozen RL baseline' throughout to 'frozen supervised stand-in for a learned controller' per reviewer critique, since the artifact is a logistic-regression contextual bandit, not RL, and was never compared to TopFull.",
    "Elevated the drift-regime result (MAD 0.0280, Holm-significant vs all three baselines) as the paper's one clean, non-fragile positive finding, distinct in evidentiary strength from the adversarial-regime result.",
    "Formalized the global-tolerance-is-not-regime-agnostic finding as a required structural finding: 3 of 5 regimes are structurally unattainable for ANY policy including the oracle, and future alpha-setting/tolerance criteria should be regime-relative.",
    "Narrowed the value-aware knapsack assumption: value must be genuinely independent of the eligibility-determining risk score for a gain to be expected; the risk-score-correlated proxy tested in iteration 2 showed a null result on the real trace, reversing an earlier self-generated-simulator positive finding.",
    "Added an open item: explain why the fixed-threshold and misspecified-index baselines converge to near-identical MAD across all regimes (likely: neither has a feedback signal, both act as static rules) rather than leaving it as an unexplained coincidence.",
    "Scoped the generality claim to the single-parameter plain-ACI update specifically, since only that variant was tested; recommended running a post-2022 strongly-adaptive or group-conditional variant as an added condition next iteration.",
    "Retained the core mechanism, the real-trace primary-evaluation methodology, and the single-threshold scope narrowing from prior iterations, since where genuinely tested (drift, matched-value comparison) the mechanism held."
  ],
  "relation_type": "evolution",
  "artifact_relations": [
    {"from_id": "art_fAlkDy9YEd-N", "to_id": "art_bHFxugXcpJMg", "relation_type": "uses", "relation_rationale": "Experiment replays 5 policies directly over the frozen real Azure-trace dataset's 210,000 rows."},
    {"from_id": "art_fAlkDy9YEd-N", "to_id": "art_k4NZEyG_q_yz", "relation_type": "uses", "relation_rationale": "Evaluation computes the primary verdict directly against the frozen real trace dataset's rows and documented rates."}
  ]
}
```

### [5] TOOL RESULT — Write · 2026-08-25 19:25:07 UTC

```
{"type": "create", "filePath": "/ai-inventor/aii_data/runs/run_GtJcfaBZUMxZ/3_invention_loop/iter_2/upd_hypo/upd_hypo/.terminal_claude_agent_struct_out.json", "content": "{\n  \"title\": \"Conformal Admission Control for Overloaded Queues\",\n  \"hypothesis\": \"An admission control policy built on ONLINE CONFORMAL RISK CONTROL -- not queueing theory, not reinforcement learning -- can hold a hard, distribution-free, finite-sample guarantee on the long-run rate of SLO violations while maximizing throughput/value, under traffic and service-time distributions that are unknown, non-stationary, and possibly adversarial -- with NO assumption of exchangeability, no fitted queueing model, and no trained neural policy.\\n\\nConcretely: at admission time, a cheap, possibly miscalibrated risk score s(x) is computed by an explicit, documented, closed-form heuristic (queue depth + coarse per-endpoint service-time estimate; exact formula and O(1) per-request cost fixed in advance and reported). The system does NOT trust s(x)'s calibration. It maintains a single scalar threshold lambda_t, updated after every observed outcome via the admission-conditional ACI gradient step lambda_{t+1} = lambda_t + eta*(alpha - y_t) (unchanged if the request was rejected, since no outcome is observed), with a FIXED, reported numeric eta (primary eta=0.05, swept over {0.01,0.02,0.05,0.10,0.20}). Admit iff s(x) <= lambda_t. This is now stated as an explicit theorem in the paper's own notation, INCLUDING the admission-conditional departure from the original ACI setting (a rejected request carries the threshold forward unchanged) and the precondition that the bound |mean(y_t) - alpha| <= B/(eta*N_T) is over N_T, the number of ADMITTED requests, not raw arrivals T -- a precondition iteration-2 evidence shows is not cosmetic: it goes vacuous exactly in the regime (adversarial, few admissions) where the paper's second-strongest claimed win lives.\\n\\nSCOPE, held from iteration 1: the evaluated and claimed guarantee is for a SINGLE shared scalar threshold over one queue/endpoint class; multi-threshold extension under a joint budget remains future work, and per reviewer feedback this narrower scope should be stated as the paper's OPENING framing (motivate from the single-queue problem outward), not narrated at fleet scale and then narrowed.\\n\\nSCOPE NARROWED FURTHER by iteration-2 real-trace evidence: the pre-registered global +/-3-percentage-point tolerance on MAD is NOT a regime-agnostic test of policy quality. Three of five regimes (stationary 3.95%, burst 0.24%, regime_switch 3.09% natural violation rate) have natural rates far BELOW the alpha=0.10 target, so no admission policy -- including the hindsight-optimal oracle -- can pass the tolerance there by construction (rejecting requests can only push the realized rate down, never up toward a target the natural rate already sits below). The guarantee is only a meaningfully testable claim in regimes where the target is a genuine constraint (natural rate at or above alpha): here, drift (15.53%) and the synthetic adversarial regime (38.25%). This structural fact, not a policy failure, must be reported as a first-class finding, and future work/iterations should set alpha relative to each regime's own natural rate (or restrict tolerance-pass claims to regimes where the criterion is well-posed) rather than reporting an aggregate pass count across regimes with structurally different answers.\\n\\nEVIDENTIARY STATUS after iteration 2 (real Azure-trace primary evaluation, closing iteration-1's self-referential-evaluation gap): conformal control is the ONLY non-oracle policy that passes the 3pp tolerance in drift (MAD 0.0280, barely inside the 0.03 line) and is Holm-significantly closer to alpha than all three non-oracle baselines there (p<0.001 each) -- this is the paper's one clean, non-fragile positive result. In adversarial, conformal's MAD (0.2418) is materially below all three baselines and Holm-significant specifically against the frozen supervised stand-in (p<0.001), but this comparison rests on as few as 56-367 admitted requests per cell out of 20,000 adversarial rows -- squarely in the regime the theorem's own precondition (B/(eta*N_T) > 1) flags as VACUOUS, and a second, independently authored replay of the same trace at the same eta grid finds the eta-sensitivity trend running in the OPPOSITE direction there. Both must therefore be treated as an unresolved, small-N artifact, not a confirmed finding, and the adversarial-regime 'win' may NOT be stated as a headline claim (Abstract/Introduction/Conclusion) without the small-N/vacuous-bound caveat attached directly alongside it, or without redesigning the adversarial regime (larger N, less extreme score separation) to keep the theorem's bound non-vacuous before it is reported as clean. Two baselines -- the fixed threshold and the misspecified queueing-index policy -- produce near-identical MAD in every regime despite being conceptually different; the next iteration/paper pass should state explicitly why (neither has a feedback signal, so both degenerate to effectively static rules on this trace), rather than leaving the near-tie unexplained. The 'frozen reinforcement-learning baseline' label is a MISNOMER the hypothesis and paper must stop using: per the artifacts, it is a single frozen logistic-regression contextual-bandit-style controller trained once on stationary traffic, not a reinforcement-learning system and not compared to TopFull or any real deep-RL controller; going forward this baseline must be called a 'frozen supervised stand-in for a learned controller' throughout, and the claim scoped to 'beats a controller that cannot adapt at all' rather than 'beats an RL controller' -- a materially weaker and more honest claim, especially since it underlies the fragile adversarial-regime win. The value-aware knapsack layer shows NO significant value gain over FCFS on the real trace and matched-violation-rate value differs from the fixed-threshold/frozen-supervised baselines by only ~0.01% (indistinguishable) while beating the misspecified index policy by ~13% (CI excludes 0) -- both are genuine results to keep, with the null knapsack result reported honestly rather than smoothed toward the earlier self-generated-simulator's positive finding. The paper evaluates only the plainest single-eta ACI update; its generality claim should be scoped explicitly to that specific update (or a strongly-adaptive/group-conditional variant [10,11] should be run as an added condition) rather than claimed as generic 'conformal admission control'.\",\n  \"motivation\": \"Unchanged from iteration 1: queueing-theoretic/index-based policies (Nino-Mora 2002) give guarantees conditional on a fitted distributional model; RL/learned controllers (TopFull, SIGCOMM 2024) adapt empirically but offer no finite-sample safety guarantee at all. Online conformal risk control (Angelopoulos et al. 2024) and Adaptive Conformal Inference (Gibbs & Candes 2021) offer a genuinely different guarantee type -- distribution-free, no exchangeability assumption -- that a targeted search still finds unapplied to systems admission control. Iteration 2 sharpens this motivation with a caveat the theorem itself supplies: the guarantee's non-asymptotic bound is only non-vacuous when enough requests are actually admitted, so the paradigm's strongest selling point (a hard number regardless of the true process) has a real, reportable failure mode -- sparse admission under an extreme adversarial score distribution -- that the paper must characterize rather than paper over.\",\n  \"assumptions\": [\n    \"A real-valued risk score s(x) is available cheaply at admission time and is at least weakly informative; iteration-2's real-trace O(1) formula (function-level service-time gap, local-vs-baseline arrival rate, queue depth, logistic-blended) satisfies this on stationary/burst/drift/regime_switch, while the adversarial regime deliberately uses a bimodal, non-model-separable score by construction.\",\n    \"The true outcome becomes observable within a bounded delay after the admission decision; iteration-2's dataset satisfies this (outcomes computed from realized service time, effectively instantaneous), and this remains an explicit, now-formalized precondition of the theorem -- a production deployment with a longer confirmation delay would need eta re-tuned against it, widening the effective window the bound applies to.\",\n    \"The per-decision loss reduces to a single bounded scalar (binary SLO-violation indicator); retained unchanged, with the theorem now explicit that the update is admission-CONDITIONAL (rejected requests carry lambda forward unchanged, unlike the original ACI setting's every-step observation).\",\n    \"Admitting/rejecting one request does not retroactively change already-admitted requests' outcomes in a way that breaks the per-decision feedback assumption; retained as an approximation to be understood per-decision-given-trajectory, not causal independence.\",\n    \"Request value is estimable at admission time for the knapsack tie-break; iteration-2 evidence NARROWS this: on the real trace, a value proxy correlated with the same risk score that determines eligibility gives the knapsack layer no significant advantage over FCFS, so the assumption should now be stated as requiring a value signal genuinely INDEPENDENT of the eligibility-determining risk score (e.g., real per-tenant billing weight) for a value gain to be expected, not merely 'value is available'.\",\n    \"NEW (from iteration-2 evidence): the theorem's finite-sample bound is only practically informative when the number of ADMITTED requests N_T is large enough that B/(eta*N_T) <= 1; a regime or score design that causes very sparse admission (as the current adversarial regime does) makes any headline comparison computed there evidentially fragile regardless of which policy 'wins' the raw MAD number.\"\n  ],\n  \"investigation_approach\": \"Phase 0/1/2/3 retained from prior iterations (mechanism check across regimes; baselines including a properly-labeled frozen supervised stand-in; score-quality sensitivity; value-aware knapsack layer), now run against the real Azure-trace dataset as PRIMARY (closing the iteration-1 self-referential-evaluation gap) with the self-generated simulator demoted to a secondary, labeled robustness check.\\n\\nNEW for the next iteration, directly targeting reviewer MAJOR findings: (a) Redesign the adversarial regime (more rows, and/or less extreme bimodal score separation so a meaningful fraction of requests remain admitted) so the theorem's own non-vacuity precondition (B/(eta*N_T) <= 1) holds at the evaluated eta grid, before reporting any adversarial-regime comparison as a headline win; alternatively, if the regime is kept as-is, every mention of the adversarial-regime result outside the Limitations section must carry the small-N/vacuous-bound caveat inline. (b) Rename the 'frozen RL baseline' consistently to 'frozen supervised stand-in for a learned controller' throughout all artifacts and the paper, and consider adding a genuinely retrained/continually-updated learned baseline as a sixth policy to test whether the drift/adversarial gaps narrow under a learned controller that CAN adapt. (c) Add a structural explanation (in the next evaluation artifact's own output, not just prose) for why the fixed-threshold and misspecified-index baselines produce near-identical MAD -- confirm/quantify that both reduce to static rules absent a feedback signal. (d) Run one of the two cited post-2022 online conformal variants (strongly-adaptive multi-window [10], or parameter-free group-conditional [11]) as an added condition in the eta-sensitivity comparison, directly testing whether it removes the need to hand-tune eta per regime, as the Discussion currently speculates. (e) Set alpha relative to each regime's own natural violation rate (or otherwise restrict pass/fail tolerance claims to regimes where the target is a genuine constraint) so the aggregate pass-rate reporting is not dominated by the three structurally-unattainable regimes.\",\n  \"success_criteria\": \"Retained from iteration 2, with the vacuous-bound caveat now load-bearing: CONFIRMING evidence is (1) conformal control passing the 3pp tolerance and beating non-oracle baselines Holm-significantly in regimes where the target is a genuine constraint (natural rate >= alpha) AND where the admitted count N_T keeps the theorem's bound non-vacuous (B/(eta*N_T) <= 1) -- a win obtained only via a sparse-admission regime does not count as confirming evidence for the headline claim without the caveat attached; (2) at matched violation rate in the stationary regime, conformal's accepted value is not worse than, and can exceed, baselines' (iteration 2: statistically indistinguishable from fixed-threshold/supervised-stand-in, ~13% better than misspecified-index, CI excludes 0 -- CONFIRMED); (3) the value-aware knapsack layer shows a significant value gain over FCFS only when tested with a value signal independent of the eligibility risk score (iteration 2's correlated proxy showed a null result -- this criterion is NOT yet met and remains open). DISCONFIRMING evidence, now made concrete by iteration 2: a claimed regime-level win that only appears where N_T is small enough to make the theorem's own bound vacuous does not, on its own, disconfirm the mechanism, but it also cannot be reported as confirming it without the redesign or caveat in (a) above. PARTIAL confirmation (matches iteration-2 status): the guarantee holds robustly and provably wins over frozen baselines specifically in regimes where the target is a genuine constraint and admission is not pathologically sparse (drift); it is untestable by a fixed global tolerance in regimes whose natural rate already sits far from target (stationary/burst/regime_switch), which is a precisely-characterized methodological finding about the evaluation criterion, not a policy failure; and it currently rests on an under-scrutinized win (adversarial) that must be either strengthened by redesign or explicitly downgraded to a caveated, non-headline result in both text and future evaluation design.\",\n  \"related_works\": [\n    \"Angelopoulos, A. N., Bates, S., et al. (2024) 'Conformal Risk Control' -- ICLR 2024 / arXiv:2208.02814. Core mechanism repurposed for admission-decision thresholding; the paper itself does not address queueing or systems SLOs.\",\n    \"Gibbs, I. & Candes, E. (2021) 'Adaptive Conformal Inference Under Distribution Shift' -- NeurIPS 2021 / arXiv:2106.00170. Source of the lambda_t online gradient update; iteration 2's paper states an explicit theorem adapting this to the admission-CONDITIONAL setting (threshold frozen on rejection) and to a bound over admitted count N_T rather than raw arrivals -- a genuine, reported departure from the original result's setting.\",\n    \"Nino-Mora, J. (2002) 'Dynamic allocation indices for restless projects and queueing admission control' -- Mathematical Programming 93. Model-based near-optimality guarantee under an assumed birth-death process; iteration 2's misspecified-index baseline instantiates this and (new finding) converges to near-identical MAD as a naive fixed threshold on the real trace absent a feedback signal, worth explaining structurally in the next iteration.\",\n    \"Park, J. et al. (2024) 'TopFull' -- ACM SIGCOMM 2024. Real deep-RL admission/rate controller with no formal guarantee; iteration 2's 'frozen RL baseline' is NOT this system but a frozen logistic-regression stand-in, and reviewer feedback requires this distinction be made explicit and consistent throughout the paper rather than only in Limitations.\",\n    \"Shahrad, M. et al. (2020) 'Serverless in the Wild' -- USENIX ATC 2020. Source trace for the 210,000-row real dataset (art_fAlkDy9YEd-N) now used as the primary evaluation, closing iteration-1's self-referential-evaluation gap.\",\n    \"Bhatnagar, A. et al. (2023) 'Improved Online Conformal Prediction via Strongly Adaptive Online Learning' -- ICML 2023 / arXiv:2302.07869, and Bharti, B. et al. (2026) 'Parameter-Free and Group Conditional Online Conformal Prediction' -- arXiv:2606.00419. Cited as motivated future-work variants in iteration 2's paper; reviewer feedback recommends actually running one of these as an added condition, since the paper's generality claim currently covers only the single-eta plain-ACI update it evaluates.\"\n  ],\n  \"inspiration\": \"Unchanged from iteration 1: CONCEPTUAL -- reframing admission thresholding from a model-based question into a pure feedback-control question over a scalar. PROCEDURAL -- importing the observe-outcome/compute-loss/gradient-step/update-threshold loop from ACI's prediction-interval workflow, repointed at admit/reject with an SLO-violation loss. METHODOLOGICAL -- combining Conformal Risk Control's threshold-selection guarantee with Gibbs & Candes' non-exchangeable online update, plus a queueing-specific score function and a value-aware knapsack layer with no direct analogue in the source literature; iteration 2 adds the explicit admission-CONDITIONAL adaptation of the update rule (threshold frozen, not updated, on rejection) as a documented, theorem-level departure from the source setting.\",\n  \"terms\": [\n    {\"term\": \"Admission control\", \"definition\": \"The policy decision of whether to accept or reject an incoming request into a queue/service system.\"},\n    {\"term\": \"Conformal risk control\", \"definition\": \"A method (Angelopoulos et al. 2024) for choosing a decision threshold that provably controls the expected value of a bounded, monotone loss at a target level, with a finite-sample, distribution-free guarantee.\"},\n    {\"term\": \"Adaptive Conformal Inference (ACI)\", \"definition\": \"An online procedure (Gibbs & Candes 2021) that updates a single threshold via a bounded gradient step after each observed outcome, provably achieving a target long-run violation frequency without exchangeability. Iteration 2's admission-control adaptation makes the update admission-conditional: the threshold is frozen (not updated) on any step where a request is rejected and no outcome is observed.\"},\n    {\"term\": \"SLO (Service Level Objective) violation\", \"definition\": \"The event that an admitted request's realized latency exceeds its target threshold.\"},\n    {\"term\": \"Distribution-free guarantee\", \"definition\": \"A statistical guarantee holding for any underlying data-generating process, in contrast to queueing-theoretic guarantees conditional on an assumed distributional model. Iteration 2 clarifies this guarantee is a bound over the number of ADMITTED requests N_T, not raw arrivals, and is vacuous (B/(eta*N_T) > 1) when too few requests are admitted.\"},\n    {\"term\": \"Risk score s(x)\", \"definition\": \"A cheap, possibly miscalibrated, real-valued function used only to rank requests by estimated SLO-violation risk; the conformal threshold, not the score's calibration, provides the formal guarantee.\"},\n    {\"term\": \"Value-aware knapsack tie-break\", \"definition\": \"Within the conformally-eligible set, admitting the highest-value requests first up to capacity rather than FCFS; iteration-2 real-trace evidence shows this yields no significant value gain when the value signal is correlated with the eligibility-determining risk score.\"},\n    {\"term\": \"Frozen supervised stand-in for a learned controller\", \"definition\": \"Iteration-2 renaming (per reviewer feedback) of what was previously called the 'frozen RL baseline': a single frozen logistic-regression contextual-bandit-style controller trained once on stationary traffic, distinct from and weaker than a genuine deep-RL system such as TopFull.\"}\n  ],\n  \"summary\": \"This hypothesis proposes ADMISSION CONTROL VIA ONLINE CONFORMAL RISK CONTROL, evaluated in iteration 2 against a real 210,000-row Azure Functions trace as the primary dataset (closing iteration 1's self-referential-evaluation gap). The controller is the only non-oracle policy passing the pre-registered tolerance in the drift regime (a clean, statistically significant win) and shows a materially lower deviation from target in the adversarial regime against baselines including a mislabeled 'frozen RL' baseline now correctly identified as a frozen logistic-regression stand-in -- but that adversarial-regime win rests on very few admitted requests and falls inside the theorem's own vacuous-bound precondition, so it must be caveated or re-earned via regime redesign rather than reported as a clean headline result. A key methodological finding is that a single global tolerance line cannot fairly test policies in regimes whose natural violation rate already sits far from the target -- even a hindsight-optimal oracle fails there by construction -- reframing what 'distribution-free control' can be shown to mean empirically. The value-aware knapsack layer shows no significant gain over FCFS with a risk-score-correlated value proxy, a genuine negative result retained rather than smoothed over.\",\n  \"_relation_rationale\": \"Same ACI frame; real-trace evidence sharpens claims, exposes vacuous-bound caveat on adversarial win, fixes RL-baseline mislabeling\",\n  \"relation_rationale\": \"Same ACI frame; narrowed adversarial-win claim to its vacuous-bound caveat, fixed RL-baseline mislabeling\",\n  \"confidence_delta\": \"unchanged\",\n  \"key_changes\": [\n    \"Attached the small-N/vacuous-bound caveat (B/(eta*N_T) > 1) directly to the adversarial-regime win as a first-class evidentiary limitation, not just a Section-5.3 footnote -- the hypothesis now states this win may not be reported as a clean headline result without either the caveat or a regime redesign that keeps the bound non-vacuous.\",\n    \"Renamed the 'frozen RL baseline' throughout to 'frozen supervised stand-in for a learned controller' per reviewer critique, since the artifact is a logistic-regression contextual bandit, not RL, and was never compared to TopFull.\",\n    \"Elevated the drift-regime result (MAD 0.0280, Holm-significant vs all three baselines) as the paper's one clean, non-fragile positive finding, distinct in evidentiary strength from the adversarial-regime result.\",\n    \"Formalized the global-tolerance-is-not-regime-agnostic finding as a required structural finding: 3 of 5 regimes are structurally unattainable for ANY policy including the oracle, and future alpha-setting/tolerance criteria should be regime-relative.\",\n    \"Narrowed the value-aware knapsack assumption: value must be genuinely independent of the eligibility-determining risk score for a gain to be expected; the risk-score-correlated proxy tested in iteration 2 showed a null result on the real trace, reversing an earlier self-generated-simulator positive finding.\",\n    \"Added an open item: explain why the fixed-threshold and misspecified-index baselines converge to near-identical MAD across all regimes (likely: neither has a feedback signal, both act as static rules) rather than leaving it as an unexplained coincidence.\",\n    \"Scoped the generality claim to the single-parameter plain-ACI update specifically, since only that variant was tested; recommended running a post-2022 strongly-adaptive or group-conditional variant as an added condition next iteration.\",\n    \"Retained the core mechanism, the real-trace primary-evaluation methodology, and the single-threshold scope narrowing from prior iterations, since where genuinely tested (drift, matched-value comparison) the mechanism held.\"\n  ],\n  \"relation_type\": \"evolution\",\n  \"artifact_relations\": [\n    {\"from_id\": \"art_fAlkDy9YEd-N\", \"to_id\": \"art_bHFxugXcpJMg\", \"relation_type\": \"uses\", \"relation_rationale\": \"Experiment replays 5 policies directly over the frozen real Azure-trace dataset's 210,000 rows.\"},\n    {\"from_id\": \"art_fAlkDy9YEd-N\", \"to_id\": \"art_k4NZEyG_q_yz\", \"relation_type\": \"uses\", \"relation_rationale\": \"Evaluation computes the primary verdict directly against the frozen real trace dataset's rows and documented rates.\"}\n  ]\n}\n", "structuredPatch": [], "originalFile": null, "userModified": false}
```

### [6] ASSISTANT · 2026-08-25 19:25:09 UTC

```
Revised hypothesis written to `.terminal_claude_agent_struct_out.json`. Key changes: attached the vacuous-bound caveat to the adversarial-regime "win," renamed the mislabeled "frozen RL baseline" to "frozen supervised stand-in," elevated the drift-regime result as the paper's clean finding, formalized the regime-tolerance structural finding, and narrowed the knapsack-value assumption based on the null real-trace result.
```
